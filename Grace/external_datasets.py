...
# Sort items by date
sorted_ds_dict = OrderedDict(sorted(ds_dict.items()))
...
for key, df in monthly_gldas_dict.items():
    relevant_columns = df.drop(columns=["index", "lat", "lon"])
...
for key, df in monthly_gldas_dict.items():
    ...
    df_filtered = df[~df[["lat", "lon"]].apply(tuple, axis=1).isin(na_lat_lon_pairs)].copy()
